...
try:
    ...
    # Create minimal Adw stubs so the app works without libadwaita
    class _AdwStub:
        class Application(Gtk.Application): pass
        class ApplicationWindow(Gtk.ApplicationWindow): pass
        class HeaderBar(Gtk.HeaderBar): pass
        ResponseAppearance = type("RA", (), {"DESTRUCTIVE": None, "SUGGESTED": None})()
        @staticmethod
        def MessageDialog(parent, title, body):
            d = Gtk.MessageDialog(transient_for=parent, message_type=Gtk.MessageType.QUESTION,
                                  buttons=Gtk.ButtonsType.NONE, text=title)
            if body: d.format_secondary_text(body)
            d._responses = {}
            def add_response(id_, label): d.add_button(label, len(d._responses)); d._responses[id_] = len(d._responses)-1
            def set_response_appearance(id_, app): pass
            def set_default_response(id_): pass
            d.add_response = add_response
            d.set_response_appearance = set_response_appearance
            d.set_default_response = set_default_response
            return d
    Adw = _AdwStub()
    HAS_ADW = False
import subprocess
